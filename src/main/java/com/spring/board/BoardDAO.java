...
public class BoardDAO implements BoardService {
	/* DAO : Data Access Object 
	 	DataBase에 CRUD하는 객체 : Select, Insert, Update, Delete*/
	 	
	 //	1. JDBC 관련 변수 선언 : Connection, Statement/PreparedStatement, ResultSet
	 	private Connection conn = null;
	 	private Statement stmt = null;
	 	private PreparedStatement pstmt = null;		//주로 사용
	 	private ResultSet rs = null;
	 	
	// 	2. SQL 쿼리를 상수에 담아서 처리, 변수 생성 후 할당 (상수명 : 전체 대문자로 사용)
	 	private final String BOARD_INSERT = 
	 					"insert into board(seq, title, write, content) values(select nvl(max(seq),0)+1 from board,?,?,?)"; 	// nvl : null을 0으로 대체
	 	private final String BOARD_UPDATE = "update board set title = ?,content=? where seq = ?";
	 	private final String BOARD_DELETE = "delete board where seq=?";
	 	private final String BOARD_GET = "select * from board where seq =?";				//DataBase 테이블에서 하나의 레코드만 출력(상세보기)
	 	private final String BOARD_LIST = "select * from board order by seq desc";				//DataBase 테이블에서 여러개의 레코드를 LIST (ArrayList()) 에 담아서 처리 
	 	
	/* 	 3. 메소드 : 
	 	insertBoard(), updateBoard(), deleteBoard()		<== 리턴 값이 없다. void
	 	getBoard() : BoardDTO에 담아서 전송 , 가져온 레코드 : 1개 
	 	getBoardList() : select한 각각의 결과를  DTO(1개) , ArrayList에 DTO 객체를 담아서 리턴
	*/
	...
	//3-1. 글 등록 처리 메소드 : insertBoard()
		...
		public void insertBoard(BoardDTO dto) {
	 		...
	 		try {
	 				//오류 발생시 프로그램이 종료되지 않도록 try catch 블락으로 처리
		...
		public void deleteBoard(BoardDTO dto) {
	 	...
	 	//3-4. 글 상세 조회 처리 메소드 : getBoard() : 레코드 1개를 DB에서 select해서 DTO객체에 담아서 "리턴" 
	 	@Override
		public BoardDTO getBoard(BoardDTO dto) {		//클라이언트 값 들어옴
	 		...
	 		try {
	 			...
	 			if(rs.next()) {		//rs의 값이 존재한다면 , rs의 값을 DTO에 담아서 리턴 (DB에서 가져오는 값은 대문자로 표시) 루프돌릴필요없어서 if문 사용
		...
		public List<BoardDTO> getBoardList(BoardDTO dto) {
	 		...
	 		try {
	 			...
	 			rs = pstmt.executeQuery();		//select쿼리
