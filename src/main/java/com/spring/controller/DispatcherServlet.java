...
public class DispatcherServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//post방식으로 변수 값 넘길 떄 한글 깨짐 방지 처리
		request.setCharacterEncoding("UTF-8");
		process(request, response);
	}
	
	// doGet, doPost의 모든 요청을 처리하는 메소드
	private void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (path.equals("/login.do")) {
			...
			// 클라이언트 요청에 대해서 /login.do (클라이언트가 login.do를 요청하면)
		...
		//	System.out.println("폼에서 넘긴 변수 pw 값 출력 : "+password); 변수값 잘 들어오는지 확인
			
		//	2. 클라이언트에서 넘긴 변수값을 받아서 저장된 변수를 DTO에 Setter 주입
			UserDTO dto = new UserDTO();
			dto.setId(id);				
			dto.setPassword(password);  //주석처리하면 null값 출력됨. dto에 id,pw 세터주입 
		...
		//	UserService user = new UserServiceImpl();
			...
			//DB의 클라이언트에서 넘긴 ID와 Password를 select해서 그 값을 DTO에 담아 리턴 시켜줌
			System.out.println(userD); 	
			
		//	4. 백엔드의 로직을 모두 처리 후 View 페이지로 이동 // null이 아니라면 select 잘 되어 rs값을 가져왔다.
			if(userD != null) {	//클라이언트에게 전송한 ID와 Password가 DB의 값과 일치 할때
				response.sendRedirect("getBoardList.do");
				System.out.println("아이디와 패스워드 일치");
			}else {//client에게 전송한 ID와 Password 중 DB값과 불일치할 떄
				response.sendRedirect("login.jsp");
				System.out.println("아이디와 패스워드 불일치");
			}
			
			
		}else if(path.equals("/getBoardList.do")) { //아이디/패스워드 일치하여 로그인이 성공했을 때 
			System.out.println("게시판 정보 출력 ");
			
			//1. Client로 부터 /getBoardList.do 요청을 받음 (게시판 정보를 출력해달라는 요청)
			
			//2. 비즈니스 로직 처리 import
			BoardDTO dto = new BoardDTO();
			BoardDAO dao = new BoardDAO();
			
			//boardList에는 DB에서 쿼리로 select한 레코드를 담은 객체가 내장되어 있다.
			List<BoardDTO> boardList = dao.getBoardList(dto);	//import
			
			//3. 클라이언트에게 boardList를 전달해야한다.
			//(세션 객체에 boardList 객체를 담아서 전송시킴)
			// 세션은 서버의 RAM에 저장됨.
			...
			//1. 클라이언트에서 넘어오는 변수값을 받아서 새로운 변수에 저장
			String title = request.getParameter("title");
			String writer = request.getParameter("writer");
			String content = request.getParameter("content");
			
			//2. 비즈니스 로직 처리 (클라리언트의 변수를 dto에 저장 후 dao의 insertBoard(dto)
			BoardDTO dto = new BoardDTO();
			BoardDAO dao = new BoardDAO();
			
			//dto의 setter 메소드 호출 시 클라리언트에게 넘어오는 변수를 할당.
			dto.setTitle(title);
			...
			System.out.println("게시판 상세 내용보기 - /getBoard.do 요청됨");
			
			
			//1. 클라이언트에서 넘긴 변수 값 받기 ("seq")
			String seq = request.getParameter("seq");	//getParameter로 넘어오는 값은 모두 String
			System.out.println("seq 변수값 : "+ seq);
			
			//2. 비즈니스 로직 처리 : parameter로 받은 값을 DTO에 저장 후 getBoard(dto)메소드 호출
			BoardDTO dto = new BoardDTO();
			BoardDAO dao = new BoardDAO();
			...
			//DB의 값이 저장된 DTO(board)를 session변수에 할당해서 view페이지로 전달
			HttpSession session = request.getSession();
			
			session.setAttribute("board",board);	// 일반적으로 객체와 변수 이름을 같이 쓴다.
...
//			System.out.println(content);
			...
			System.out.println("사용자 로그아웃 정보처리");
